...
This command runs nine fixed release-blocking production paths:

1. Production Agent editing and verification
2. Structured output
...
These paths require evidence from a real Provider or production host. Model self-reporting, HTTP `200`, mocked ToolExecutors, and jsdom-only coverage are not substitutes. Provider admission, retries, compaction, queueing, Session identity, event projection, and resource cleanup remain release-blocking through deterministic unit and integration tests. Open-ended multi-file migration, process trees, concurrent ownership, crash tails, and other high-cost provider/capability soaks run separately via:

```bash
bun run test:real-api
```

Domestic model channels are optional soak providers and do not enter the default release-blocking matrix. To explicitly include them, set `REAL_API_INCLUDE_OPTIONAL_PROVIDERS=1`. The extended matrices described by capability below define the complete soak contract; they do not imply that every patch must block on every soak.
...
This soak trajectory has framework retry set to 0 when `REAL_API_RELEASE_MATRIX=1`. Desktop computer-use can only serve as non-blocking visual observation; it cannot prove JSONL, Provider request order, or marker non-fan-out, therefore it is not the authority for this runtime contract.
...
The required matrix fixedly includes `deepseek-v4-flash` and `deepseek-v4-pro`, four surfaces totaling eight cells. This trajectory is a full real API soak, and must not be substituted by mocks, HTTP 200, JSONL-only checks, or incidental visibility after refresh.
...
The required matrix fixedly includes `deepseek-v4-flash` and `deepseek-v4-pro`, four surfaces totaling eight cells. This trajectory is a full real API soak, and must not be substituted by mock signals, directly calling `SessionRuntime.dispose()`, only checking process exit codes, or cold `process_restart` repair.
...
The full-soak active-turn matrix runs DeepSeek Flash/Pro across Headless, real
