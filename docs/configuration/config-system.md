...
当前消息或历史中含有用户图片时，fallback 也必须支持图片输入；遇到纯文本模型时会在发送请求前报错，不会丢弃图片继续执行。请为这类任务配置支持图片的 fallback。

## 图片附件下载

HTTP(S) 图片在发送给 Provider 前下载。每条用户消息的图片共用 5 MiB
编码预算，包含内联图片、下载内容的 base64 编码及 data URL 前缀；下载按实际读取的
字节计量，不依赖服务端声明的大小。每张远程图片的下载时限为 30 秒，包含响应正文。
超限、超时或取消会中止同批下载，并在释放响应流后结束该次请求，不会发送不完整的
图片给 Provider。回退复用已经解析的图片，不重复下载。工具产生的内联截图仍遵循
工具自身的 artifact 大小限制，不占用用户附件预算。
