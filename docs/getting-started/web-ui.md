...
停止当前轮次会保留尚未处理的输入。之后在当前会话切换模型并发送新消息时，Web 会先
建立仅接收事件的连接，再提交新消息与模型选择，避免旧输入抢先使用上一个模型恢复。
重新进入会话或已建立连接的断线重连仍保留待处理任务的自动恢复行为。旧连接关闭后的
迟到回调不会更新新连接；事件必须通过结构和会话身份校验后才能推进重连游标。正常
心跳仅保活，不改变持久化事件游标。每次重连等待服务器握手最多 10 秒，超时后按
既有退避继续重试；连续重连失败达到预算后显示离线，而不是永久停留在“重连中”。
网络恢复后可点击离线提示中的“重新连接”重新建立事件订阅并恢复保留的待处理输入；
连接耗尽本身不会删除这些输入。

图片上传超出附件预算时，错误提示不会清空文字草稿。执行失败后可使用“编辑后重发”
恢复原消息和图片附件。
...
“日志”标签支持“轨迹”和“明细”两种视图。轨迹视图按运行、模型调用、工具调用和
缓存命中四条泳道展示会话活动，并汇总预估成本、缓存输入与新输入；这些指标随
会话持久化，重新打开会话后仍可查看。明细视图继续提供工具输出搜索与状态筛选。
