...
回合结果的 `tokensUsed` 累计该循环中已返回的 Provider 用量，包括阈值压缩、反应式压缩
和轮次上限压缩的摘要请求；降级、后续持久化失败或取消不会抹去已经报告的消耗。它与
`token_usage` 事件使用相同的规范化计数，供现有 Goal 预算在回合结束时累计并持久化。
尚未返回的 Provider 消耗不作估算，也不把子代理或其他独立循环的用量重复计入当前循环。
累计消耗不是当前上下文长度，不改变压缩阈值、输出续写或传输重试预算。

自动压缩摘要的 `token_usage` / Web `token.usage` 事件带 `scope: auxiliary`，Headless
JSONL 保留同名字段。TUI/Web 累计其费用和用量，但不覆盖主请求的上下文占用或窗口上限；
取消、采样失败或 checkpoint 失败保留原读数，只有压缩成功提交后才清零旧上下文读数。

空摘要耗尽、后续采样错误或摘要后处理失败而返回降级结果时，仍保留此前已返回的摘要
用量，包括 reasoning、缓存和费用字段。重试等待、后续采样或摘要后处理阶段取消时，
仍上抛 `AbortError`，保留原始异常为 cause，并携带此前已返回的累计用量；循环只累计一次，
不生成降级结果或压缩 checkpoint，也不增加压缩熔断失败次数。取消前尚未返回的用量仍不作估算。

阈值、反应式和轮次上限压缩均继承当前 Session 的模型目录、渠道请求头、认证与端点，
不会重新查询全局同名渠道。摘要温度、输出上限、超时及采样重试预算仍独立于主请求。

摘要响应开头的 `<analysis>` 段不会进入压缩上下文，其中引用的 `<summary>` 标签也不用于
提取摘要。分析未闭合或移除分析后没有公开摘要时，沿用空摘要有界重试与降级，不丢弃已返回用量。
