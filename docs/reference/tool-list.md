...
`ToolSearch` 可用时，扩展工具保持按需加载。若白名单或黑名单移除了 `ToolSearch`，
Runtime 会直接提供当前注册表中已获准工具的完整 schema，不会额外加入加载器或放宽
执行权限。Plan 模式仍只暴露只读工具；MCP 目录新增、替换或移除后使用相同规则。

若活动 Skill 的 `allowed-tools` 排除了 `ToolSearch`，请求会直接携带注册表与当前模式
允许、且 Skill 允许的完整 schema。清除 Skill 限制后恢复原有延迟加载，不把临时投影
标记为永久加载；结构化输出保留工具不受影响。
