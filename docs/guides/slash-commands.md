...
TUI 和 Web 会累计已完成旁路请求的 Token、缓存及费用，但不会用旁路响应覆盖主会话
状态栏的上下文占用；主回合更新或压缩清零后，较晚返回的旁路结果也不会恢复旧读数。
TUI 旁路问题头部采用单行预览，长文本按终端宽度省略，换行仅在展示时折叠；发送给
Provider 的原始问题不会截断，缩窄终端也不会让问题头部挤走短答案和主输入区。
旁路答案和错误信息使用随终端高度调整的滚动区域，保留 Markdown；主输入框获得焦点时
按 `PgUp` / `PgDn` 翻页，普通输入和方向键仍用于主输入框。新问题从顶部显示，关闭面板、
打开弹窗或进入历史浏览时，旁路不会接收翻页键。
旁路回答完成或报错后，主输入框获得焦点时按 `Esc` 只关闭旁路面板，保留主输入草稿，
不会中断仍在运行的主任务；后续 `Esc` 恢复主输入原有行为。加载中的旁路取消方式不变。
...
TUI 的 Esc 和 ACP cancel 会中止尚未完成的手动摘要请求。取消在 checkpoint 提交前被
观察到时，不替换原上下文或写入项目记忆；已返回用量仍交付，TUI 累计消耗但不清零上下文
读数。checkpoint 一旦提交，后续取消不回滚已持久化的压缩结果。Web 尚无独立的手动压缩入口。

ACP 手动压缩使用当前 Session 选择的模型、上下文窗口和独立渠道配置，而不是进程的全局
默认模型。Session 的渠道解析失败时直接报错，不改用全局模型；摘要采样预算保持独立。
