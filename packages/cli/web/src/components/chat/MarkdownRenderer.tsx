...
const LazyCodeBlockHighlighter = lazy(() =>
  import('./CodeBlockHighlighter').then((module) => ({
    default: module.CodeBlockHighlighter,
  }))
);
...
export const MarkdownRenderer = memo(function MarkdownRenderer({
  ...
  const components: Components = useMemo(
    () => ({
        ...
        return (
          <div className="relative my-4 rounded-lg overflow-hidden border border-[#E5E7EB] dark:border-[#27272a] bg-[#F9FAFB] dark:bg-[#1e1e1e]">
            <div className="flex items-center justify-between px-4 py-2 bg-[#F3F4F6] dark:bg-[#27272a] border-b border-[#E5E7EB] dark:border-[#27272a]">
              <span className="text-xs text-[#6B7280] dark:text-[#a1a1aa] font-mono">
                {language || 'text'}
              </span>
      ...
      ol({ children }) {
        return (
          <ol className="pl-4 mb-4 space-y-1 list-decimal last:mb-0">{children}</ol>
        );
      },
      li({ children }) {
      ...
      h1({ children }) {
        return (
          <h1 className="text-2xl font-bold mb-4 mt-6 text-[#111827] dark:text-[#E5E5E5]">
            {children}
          </h1>
        );
      },
      h2({ children }) {
        return (
          <h2 className="text-xl font-bold mb-3 mt-5 text-[#111827] dark:text-[#E5E5E5]">
            {children}
          </h2>
        );
      },
      h3({ children }) {
        return (
          <h3 className="text-lg font-bold mb-2 mt-4 text-[#111827] dark:text-[#E5E5E5]">
            {children}
          </h3>
        );
      },
      blockquote({ children }) {
        return (
          <blockquote className="border-l-2 border-[#22C55E] pl-4 italic text-[#6B7280] dark:text-[#a1a1aa] mb-4">
            {children}
          </blockquote>
        );
      },
      ...
      table({ children }) {
        return (
          <div className="overflow-x-auto mb-4 border border-[#E5E7EB] dark:border-[#27272a] rounded-lg">
      ...
      thead({ children }) {
        return (
          <thead className="bg-[#F3F4F6] text-[#111827] dark:bg-[#27272a] dark:text-[#E5E5E5]">
            {children}
          </thead>
        );
      },
      ...
      tr({ children }) {
        return (
          <tr className="hover:bg-[#F3F4F6] dark:hover:bg-[#27272a]/50 transition-colors">
            {children}
          </tr>
        );
      },
      ...
      td({ children }) {
        return (
          <td className="px-4 py-3 text-[#374151] dark:text-[#d4d4d8]">{children}</td>
        );
      },
      hr() {
  ...
  return (
    <div
      className={cn(
        'text-[14px] text-[#111827] dark:text-[#E5E5E5] font-mono break-words min-w-0 w-full',
        className
      )}
    >
