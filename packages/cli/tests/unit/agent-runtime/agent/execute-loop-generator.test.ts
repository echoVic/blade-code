...
describe('executeLoopGenerator', () => {
  ...
  it('propagates an Agent Team task-list scope to tool execution', async () => {
    ...
    chatMock
      .mockResolvedValueOnce(
        namedToolResponse(
          'TaskCreate',
          '{"subject":"Shared","description":"Team task"}',
          'tc-shared-task-list',
  ...
  it('propagates the host turn ID to non-streaming tool execution', async () => {
    ...
    chatMock
      .mockResolvedValueOnce(
        namedToolResponse('Read', '{"path":"package.json"}', 'tc-host-turn', {
          promptTokens: 100,
          completionTokens: 20,
          totalTokens: 120,
        })
  ...
  describe('Skill deferred tool projection', () => {
    ...
    it('retains the reserved structured output schema while exposing only Skill-admitted tools', async () => {
      ...
      const chat = vi
        .mocked(deps.chatService.chat)
        .mockResolvedValueOnce(
          namedToolResponse('StructuredOutput', '{"answer":"done"}', 'structured-skill')
        )
  ...
  describe('simple text response (no tool calls)', () => {
    ...
    it('persists prompt artifact metadata while evaluating host policy from the original input', async () => {
      ...
      chatMock
        ...
        .mockResolvedValueOnce(
          namedToolResponse('Bash', '{"command":"npm test"}', 'verify-offloaded-input')
        )
        .mockResolvedValueOnce({
  ...
  it('enforces an explicit turn limit for the main agent in yolo mode', async () => {
    ...
    const chatMock = deps.chatService.chat as ReturnType<typeof vi.fn>;
    chatMock.mockResolvedValueOnce(
      namedToolResponse('Bash', '{"command":"echo retry"}', 'tc-main-turn-limit')
    );
    (deps.toolExecutor.execute as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      success: false,
      llmContent: 'blocked',
  ...
  it('turn-limit continuation 原样持久化 CompactionService boundary 的无 marker replacement', async () => {
    ...
    chatMock
      .mockResolvedValueOnce(
        namedToolResponse('Read', '{"path":"frontier.ts"}', 'tc-turn-limit-marker', {
          promptTokens: 100,
          completionTokens: 20,
          totalTokens: 120,
        })
  ...
  describe('tool call → tool result → final response (2 turns)', () => {
    it('returns a typed Bash execution-host failure for the logical turn', async () => {
      ...
      chatMock
        .mockResolvedValueOnce(
          namedToolResponse('Bash', '{"command":"true"}', 'bash-host-failure')
        )
    ...
    it('does not force a tool that already produced a successful result', async () => {
      ...
      chat
        .mockResolvedValueOnce(toolResponse(100))
        .mockResolvedValueOnce(finalResponse(120, TEXTUAL_READ_CALL))
        .mockResolvedValueOnce(
    ...
    it('fails instead of accepting repeated textual tool calls as successful completion', async () => {
      const { deps, saveToolUse, saveMessage, chat } = createTextualToolHarness();
      chat.mockResolvedValue(finalResponse(100, TEXTUAL_READ_CALL));
    ...
    it('allows tool-call JSON requested as example output without executing it', async () => {
      ...
      expect(result).toMatchObject({
        success: true,
        finalMessage: TEXTUAL_READ_CALL,
      });
      expect(chat).toHaveBeenCalledOnce();
      expect(deps.toolExecutor.execute).not.toHaveBeenCalled();
    });
    ...
    it('does not exceed an explicit turn limit to correct textual tool calls', async () => {
      const { deps, saveMessage, chat } = createTextualToolHarness();
      deps.runtimeOptions.maxTurns = 1;
      chat.mockResolvedValue(finalResponse(100, TEXTUAL_READ_CALL));
      const { result } = await runTextualToolLoop(deps);
      expect(result).toMatchObject({
    ...
    it('corrects streamed textual tool calls only through native tool execution', async () => {
      const { deps, saveToolUse } = createTextualToolHarness();
      ...
      const { result } = await runTextualToolLoop(deps, { stream: true });
      expect(result).toMatchObject({
        success: true,
    ...
    it('does not promote textual calls for tools excluded by the current policy', async () => {
      ...
      chat.mockResolvedValue(finalResponse(100, TEXTUAL_READ_CALL));
      const { result } = await runTextualToolLoop(deps);
      expect(result.finalMessage).toBe(TEXTUAL_READ_CALL);
      expect(chat).toHaveBeenCalledOnce();
      expect(deps.toolExecutor.execute).not.toHaveBeenCalled();
      expect(saveToolUse).not.toHaveBeenCalled();
    });
    ...
    it('continues until every explicitly requested verification category succeeds', async () => {
      ...
      chatMock
        ...
        .mockResolvedValueOnce(
          namedToolResponse('Bash', '{"command":"npm run type-check"}', 'tc-type-check')
        )
        .mockResolvedValueOnce({
    ...
    it('propagates a worktree workspace transition to later tool calls', async () => {
      ...
      chatMock
        ...
        .mockResolvedValueOnce(
          namedToolResponse('Bash', '{"command":"pwd"}', 'tc-bash', {
            promptTokens: 120,
            completionTokens: 20,
            totalTokens: 140,
          })
  ...
  it('refreshes provider declarations after ToolSearch activates a deferred tool', async () => {
    ...
    chatMock
      .mockResolvedValueOnce(
        namedToolResponse(
          'ToolSearch',
          '{"query":"select:LSP","max_results":1}',
          'load-lsp'
        )
      )
      .mockResolvedValueOnce(
