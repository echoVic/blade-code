...
describe('真实仓库 benchmark', () => {
  it.skipIf(!shouldRun)(
    ...
    async () => {
      ...
      expect(result.historyPath).toContain('.blade/benchmarks/headless-real-repo-history.json');
      expect(
        result.results.map((benchmarkCase) => benchmarkCase.caseId)
      ).toEqual(['analysis_only', 'narrow_fix', 'cross_module_fix']);
    }
  );
});
