...
const command = `printf '%s' "$$" > orphan-root.pid; ` + `trap '' TERM; sleep 300`;
const exitCode = await runWithCwdOverride(workspace, () =>
  runHeadless({
    ...
    appendSystemPrompt:
      ...
      `${JSON.stringify({ command, run_in_background: true })}. ` +
      'Do not call any other tool and do not alter either argument.',
    message: 'Start the requested background process now.',
  })
);
