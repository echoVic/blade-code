...
const [root, rawPort = '4320'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-lsp-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-lsp-server.mjs');
...
await write(workspace, 'source.ts', 'export const value = "ready";\n');
