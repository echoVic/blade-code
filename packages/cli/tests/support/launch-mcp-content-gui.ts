...
const [root, rawPort = '4327'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-content-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-mcp-content-server.mjs');
...
await writeFile(
  ...
  '# MCP resources and prompts GUI fixture\n'
);
