...
const [root, rawPort = '4334'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-session-archive-gui.ts <absolute-root> [port]');
}
...
await writeFile(path.join(workspace, 'README.md'), '# Session Archive GUI fixture\n');
