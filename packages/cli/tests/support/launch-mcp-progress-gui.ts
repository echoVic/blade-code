...
const [root, rawPort = '4324'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-progress-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-mcp-lifecycle-server.mjs');
...
await writeFile(path.join(workspace, 'README.md'), '# MCP progress fixture\n');
