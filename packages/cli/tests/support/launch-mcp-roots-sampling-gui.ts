...
const [root, rawPort = '4323'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-roots-sampling-gui.ts <absolute-root> [port]');
}

const port = Number(rawPort);
const execFileAsync = promisify(execFile);
const home = path.join(root, 'home');
const workspace = path.join(root, 'project with space');
const storage = path.join(root, 'storage');
const pidFile = path.join(root, 'mcp.pid');
const fakeServer = path.resolve(
  import.meta.dirname,
  'fake-mcp-roots-sampling-server.mjs'
);
...
await writeFile(
  ...
  '# MCP roots and sampling fixture\n'
);
