...
const [root, rawPort = '4333'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-tasks-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-mcp-task-server.mjs');
...
await writeFile(path.join(workspace, 'README.md'), '# MCP Tasks GUI fixture\n');
