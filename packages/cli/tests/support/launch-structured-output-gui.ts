...
const [root, rawPort = '4342'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-structured-output-gui.ts <absolute-root> [port]');
}
...
await writeFile(
  ...
  '# Structured output GUI qualification\n'
);
