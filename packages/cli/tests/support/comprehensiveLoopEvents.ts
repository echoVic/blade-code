...
export function comprehensiveLoopEvents(): LoopEvent[] {
  const task = {
    ...
    description: 'Exercise the event contract',
  ...
  const goal = {
    ...
    goalId: 'goal-1',
    objective: 'Exercise every projection',
  ...
  return [
    { kind: 'turn_start', turn: 1, maxTurns: 4 },
    { kind: 'content_delta', delta: 'answer' },
    { kind: 'thinking_delta', delta: 'reasoning' },
    { kind: 'stream_end' },
    {
      kind: 'tool_start',
      toolCall: toolCall('Grep', { pattern: 'needle', path: 'src' }),
      toolKind: 'readonly',
    },
    {
      kind: 'tool_progress',
      toolCall: toolCall('Grep', { pattern: 'needle' }),
      update: {
        message: 'searching',
        progress: 1,
        total: 2,
    ...
    },
    {
      ...
      result: {
        success: true,
        llmContent: 'match',
        metadata: { summary: 'Found match' },
      },
    },
    {
      kind: 'tool_start',
      toolCall: toolCall('Task', {
    ...
    },
    {
      ...
      toolCall: {
        ...toolCall('Task', {}),
        function: { name: 'Task', arguments: '{invalid' },
      },
    },
    {
      kind: 'tool_result',
      toolCall: toolCall('Edit', { file_path: 'src/demo.ts' }),
      result: {
        success: true,
        llmContent: 'edited',
        metadata: {
    ...
    {
      kind: 'tool_result',
      toolCall: toolCall('ApplyPatch', {}, 'patch'),
      result: {
        success: true,
        llmContent: 'patched',
        metadata: {
    ...
    {
      kind: 'tool_result',
      toolCall: toolCall('Task', {}, 'verify'),
      result: {
        success: false,
        llmContent: 'partial',
        error: { type: 'execution_error', message: 'incomplete' },
    ...
    },
    {
      ...
      outcome: 'compacted',
      preTokens: 90,
      preTokenSource: 'provider',
      estimatedPendingTokens: 12,
      postTokens: 40,
      sampleAttempts: 2,
      inputReductions: 1,
      messagesOmitted: 3,
      filesOmitted: 1,
      imagesOmitted: 1,
      fallbackTargetTokens: 30,
      fallbackMessagesOmitted: 2,
    ...
    { kind: 'task_update', tasks: [task] },
    {
      kind: 'goal_frontier_updated',
      goal: {
        ...goal,
        frontierStall: {
          category: 'no_progress' as const,
          consecutiveCount: 2,
          updatedAt: '2026-09-16T00:00:01.000Z',
        },
      },
    ...
    { kind: 'follow_up_queue_changed', queue },
    {
