...
function loadInput(): RunnerInput {
  const encoded = process.env.BLADE_WEIGHTED_ADMISSION_ACP_INPUT;
  if (!encoded) throw new Error('Missing BLADE_WEIGHTED_ADMISSION_ACP_INPUT');
...
function waitForChildExit(
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      cleanup();
      reject(new Error('Weighted ACP admission child did not exit'));
    }, timeoutMs);
    const cleanup = () => {
      clearTimeout(timer);
...
async function run(input: RunnerInput) {
  ...
  if (!child.stdin || !child.stdout) {
    child.kill('SIGKILL');
    throw new Error('Weighted ACP admission stdio was unavailable');
  }
  let stderr = '';
  ...
  try {
    ...
    }, 'Weighted ACP primary request did not reach the Provider hold barrier');
    ...
    const secondaryOutcome = connection
      .prompt({
        sessionId: secondarySessionId,
        prompt: [
          {
            type: 'text',
            text:
              'This request must be rejected before Provider traffic. ' +
      ...
      () =>
        admissionMetadata(client, secondarySessionId).some(
          (value) =>
            ...
            typeof value === 'object' &&
            !Array.isArray(value) &&
            (value as Record<string, unknown>).phase === 'rejected' &&
            (value as Record<string, unknown>).resource === 'pending_bytes' &&
            (value as Record<string, unknown>).reason === 'queue_full'
        ),
      'Weighted ACP secondary Session did not project pending-byte rejection'
    );
    ...
    if (primaryResult.stopReason !== 'end_turn') {
      throw new Error(
        `Unexpected weighted ACP primary stop reason: ${primaryResult.stopReason}`
      );
    }
    if (secondaryResult.kind !== 'error') {
      throw new Error(
        `Weighted ACP secondary unexpectedly completed: ${secondaryResult.result.stopReason}`
      );
    }
    ...
    if (!metadata.includes(null)) {
      throw new Error('Weighted ACP admission metadata was not cleared');
    }
    const primaryText = agentText(client, primarySessionId);
    const secondaryText = agentText(client, secondarySessionId);
    if (!primaryText.includes(input.primaryMarker)) {
      throw new Error('Weighted ACP primary Session did not finish independently');
    }
    if (
      primaryText.includes('providerAdmission') ||
      secondaryText.includes('providerAdmission')
    ) {
      throw new Error('Weighted ACP admission metadata polluted assistant text');
    }
    ...
    for (const value of [
      ...
      if (value.includes(input.secret)) {
        throw new Error('Weighted ACP admission evidence exposed credentials');
      }
    }
