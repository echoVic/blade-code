...
const [root, rawPort = '4335'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-session-export-gui.ts <absolute-root> [port]');
}
...
await writeFile(path.join(workspace, 'README.md'), '# Session Export GUI fixture\n');
await writeFile(
