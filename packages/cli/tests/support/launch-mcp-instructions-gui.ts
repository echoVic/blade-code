...
const [root, rawPort = '4331'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-instructions-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(
  import.meta.dirname,
  'fake-mcp-instructions-server.mjs'
);
...
await writeFile(
  ...
  '# MCP server instructions GUI fixture\n'
);
