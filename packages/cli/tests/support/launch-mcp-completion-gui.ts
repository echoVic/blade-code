...
const [root, rawPort = '4332'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-completion-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-mcp-completion-server.mjs');
...
await writeFile(path.join(workspace, 'README.md'), '# MCP completion GUI fixture\n');
