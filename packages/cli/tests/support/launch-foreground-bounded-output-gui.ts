...
const [root, rawPort, modelName] = process.argv.slice(2);
if (
  !root ||
  ...
  throw new Error(
    'Usage: bun launch-foreground-bounded-output-gui.ts <absolute-root> <port> <model>'
  );
}
...
const model = resolveForkQualificationModels(process.env, {
  requiredDeepSeek: true,
}).find((candidate) => candidate.id === 'deepseek' && candidate.model === modelName);
if (!model) throw new Error(`Qualification model is unavailable: ${modelName}`);
...
await writeFile(
  ...
  '# Bounded foreground output browser qualification\n'
);
