...
const [root, rawPort = '4330'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-logging-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-mcp-logging-server.mjs');
...
await writeFile(path.join(workspace, 'README.md'), '# MCP logging GUI fixture\n');
