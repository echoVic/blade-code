...
const [root, rawPort = '4329'] = process.argv.slice(2);
if (!root || !path.isAbsolute(root) || !/^\d+$/.test(rawPort)) {
  throw new Error('Usage: bun launch-mcp-tool-result-gui.ts <absolute-root> [port]');
}
...
const fakeServer = path.resolve(import.meta.dirname, 'fake-mcp-tool-result-server.mjs');
...
await writeFile(
  ...
  '# MCP tool result safety GUI fixture\n'
);
