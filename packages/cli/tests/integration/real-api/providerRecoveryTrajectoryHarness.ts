...
const allSurfaces = ['headless', 'acp', 'pty', 'web'] as const;
type ProviderRecoverySurface = (typeof allSurfaces)[number];

interface ProviderRecoveryScenario {
  title: string;
  surfaces: readonly ProviderRecoverySurface[];
  expectedCells: number;
  requireReleaseMatrix: boolean;
  injectedFailures: number;
  failureStatus: 429 | 503;
  failureType: 'rate_limit_error' | 'server_error';
  failureReason: 'rate_limit' | 'server_error';
  retryAfterMs(failure: number): number;
  privateBodyMarker: string;
  packageName: string;
  rootPrefix: string;
  sessionPrefix: string;
  markerPrefix: string;
  secondaryMarkerPrefix: string;
  promptLabel: string;
  contaminationLabel: string;
  secondaryContaminationLabel: string;
  retryAttempts: readonly number[];
  recoveredAttempt: number;
  recoveryActivities: readonly string[];
  webBannerText: string;
  frameworkRetryDisabled: boolean;
  expectTurnActivity: boolean;
  acpPayload: Readonly<Record<string, unknown>>;
  ptyPayload: Readonly<Record<string, unknown>>;
  expectPtyActivity: boolean;
}

const scenarios = {
  recovery: {
    title: 'bounded foreground Provider recovery release matrix',
    surfaces: releaseBlockingSurfaces(allSurfaces),
    expectedCells: isReleaseMatrix() ? 6 : 8,
    requireReleaseMatrix: false,
    injectedFailures: 4,
    failureStatus: 503,
    failureType: 'server_error',
    failureReason: 'server_error',
    retryAfterMs: (failure) => (failure === 4 ? 2_000 : 0),
    privateBodyMarker: 'PRIVATE_FOREGROUND_RECOVERY_BODY_MUST_NOT_SURFACE',
    packageName: 'blade-foreground-provider-recovery',
    rootPrefix: 'blade-provider-recovery',
    sessionPrefix: 'provider-recovery',
    markerPrefix: 'PROVIDER_RECOVERY_OK',
    secondaryMarkerPrefix: 'PROVIDER_CIRCUIT_SHARED_OK',
    promptLabel: 'bounded foreground provider recovery',
    contaminationLabel: 'Provider recovery final marker contaminated the prompt',
    secondaryContaminationLabel:
      'Secondary Provider recovery marker contaminated the prompt',
    retryAttempts: [1, 2, 3, 4],
    recoveredAttempt: 4,
    recoveryActivities: ['retry_attempt', 'circuit_open', 'circuit_probe'],
    webBannerText: 'Provider isolated',
    frameworkRetryDisabled: false,
    expectTurnActivity: false,
    acpPayload: {},
    ptyPayload: {},
    expectPtyActivity: false,
  },
  rateLimit: {
    title: 'authoritative Provider rate-limit cooldown release matrix',
    surfaces: allSurfaces,
    expectedCells: 8,
    requireReleaseMatrix: true,
    injectedFailures: 1,
    failureStatus: 429,
    failureType: 'rate_limit_error',
    failureReason: 'rate_limit',
    retryAfterMs: () => 2_000,
    privateBodyMarker: 'PRIVATE_RATE_LIMIT_COOLDOWN_BODY_MUST_NOT_SURFACE',
    packageName: 'blade-rate-limit-cooldown',
    rootPrefix: 'blade-rate-limit-cooldown',
    sessionPrefix: 'rate-limit-cooldown',
    markerPrefix: 'PROVIDER_RECOVERY_OK',
    secondaryMarkerPrefix: 'PROVIDER_CIRCUIT_SHARED_OK',
    promptLabel: 'authoritative Provider rate-limit cooldown',
    contaminationLabel: 'Rate-limit cooldown final marker contaminated the prompt',
    secondaryContaminationLabel:
      'Secondary rate-limit cooldown marker contaminated the prompt',
    retryAttempts: [1],
    recoveredAttempt: 1,
    recoveryActivities: ['circuit_open', 'circuit_probe'],
    webBannerText: 'Provider rate limited · waiting for recovery probe',
    frameworkRetryDisabled: true,
    ...
    expectPtyActivity: true,
  },
} satisfies Record<string, ProviderRecoveryScenario>;

export type ProviderRecoveryScenarioName = keyof typeof scenarios;
...
async function startRecoveryProxy(baseUrl: string, scenario: ProviderRecoveryScenario) {
  let requestCount = 0;
  let injectedFailures = 0;
  let forwardedRequests = 0;
  let circuitOpenedAt: number | undefined;
  const requestStartedAt: number[] = [];
  const privateBodyMarker = scenario.privateBodyMarker;
  const server = createServer((request, response) => {
    void (async () => {
      const body = await readRequestBody(request);
      requestCount++;
      requestStartedAt.push(Date.now());
      if (injectedFailures < scenario.injectedFailures) {
        injectedFailures++;
        response.writeHead(scenario.failureStatus, {
          'content-type': 'application/json',
          'retry-after-ms': String(scenario.retryAfterMs(injectedFailures)),
        });
        response.end(
          JSON.stringify({
            error: {
...
function assertHeadlessRetry(stdout: string, scenario: ProviderRecoveryScenario): void {
  const events = stdout
    .split(/\r?\n/)
    .filter(Boolean)
    .flatMap((line) => {
  ...
  ).toEqual(scenario.retryAttempts);
  expect(retryEvents).toContainEqual(
    expect.objectContaining({
      phase: 'recovered',
      attempt: scenario.recoveredAttempt,
      max_retries: 12,
  ...
  expect(circuitEvents[1]).toMatchObject({
    ...
    status_code: scenario.failureStatus,
    retry_after_ms: CIRCUIT_OPEN_MS,
    open_duration_ms: CIRCUIT_OPEN_MS,
  });
  expect(recoveryEvents).toEqual(
    expect.arrayContaining([
      ...scenario.recoveryActivities.map((activity) =>
        expect.objectContaining({
          snapshot: expect.objectContaining({ activity }),
        })
      ),
  ...
  if (scenario.expectTurnActivity) {
...
async function runHeadless(input: {
  ...
  marker: string;
  secret: string;
  scenario: ProviderRecoveryScenario;
  ...
  try {
    ...
    assertHeadlessRetry(stdout, input.scenario);
...
async function runWeb(input: {
  ...
  try {
      ...
      () =>
        probe?.events.some(
          (event) =>
            ...
            event.properties.attempt === input.scenario.recoveredAttempt &&
            event.properties.maxRetries === 12 &&
            event.properties.mode === 'bounded_foreground'
        ) === true,
      'Web SSE did not project the bounded Provider retry',
      60_000
    );
    await waitFor(
      () =>
    ...
    expect(await recoveryBanner.textContent()).toContain(input.scenario.webBannerText);
...
export function defineProviderRecoveryTrajectory(
  scenarioName: ProviderRecoveryScenarioName
): void {
  ...
  const matrix = models.flatMap((model) =>
    scenario.surfaces.map((surface) => ({ model, surface }))
  );
  if (isRealApiTestEnabled() && matrix.length !== scenario.expectedCells) {
    throw new Error(
      `${scenario.title} must contain ${scenario.expectedCells} cells, got ${matrix.length}`
    );
  }
  if (isRealApiTestEnabled() && scenario.requireReleaseMatrix && !isReleaseMatrix()) {
    throw new Error(`${scenario.title} requires REAL_API_RELEASE_MATRIX=1`);
  }

  describe.skipIf(!isRealApiTestEnabled()).sequential(scenario.title, () => {
    for (const { model, surface } of matrix) {
      it(`${model.model} × ${surface}`, async (context) => {
        if (scenario.frameworkRetryDisabled) {
          expect(frameworkRetryBudget(context)).toBe(0);
        }
        const root = await mkdtemp(
          path.join(
            os.tmpdir(),
            `${scenario.rootPrefix}-${safeSlug(model.model)}-${surface}-`
          )
        );
        ...
        let sessionId = `${scenario.sessionPrefix}-${safeSlug(
          model.model
        )}-${surface}-${Date.now()}`;
        const marker = `${scenario.markerPrefix}_${Date.now()}`;
        const secondaryMarker = `${scenario.secondaryMarkerPrefix}_${Date.now()}`;
        const prompt = [
          `[PASTE: ${scenario.promptLabel}]`,
        ...
        try {
          ...
          const surfaceRunners: Record<
            ProviderRecoverySurface,
            () => Promise<SurfaceEvidence>
          > = {
            headless: () =>
              runHeadless({
          ...
          const evidence = await surfaceRunners[surface]();
          if (surface === 'acp' || surface === 'web') {
            sessionId = evidence.sessionId;
          }

          expect(proxy.injectedFailures()).toBe(scenario.injectedFailures);
          expect(proxy.requestCount()).toBeGreaterThanOrEqual(
            scenario.injectedFailures + 1
          );
          expect(proxy.forwardedRequests()).toBeGreaterThanOrEqual(1);
          const circuitOpenedAt = proxy.circuitOpenedAt();
          const firstPostOpenRequestAt =
            proxy.requestStartedAt()[scenario.injectedFailures];
          ...
          if (surface === 'pty') {
            ...
            if (
              scenario.expectTurnActivity &&
              (surface === 'acp' || surface === 'web')
            ) {
