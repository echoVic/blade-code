...
describe
  ...
  .sequential('bounded Session Runtime residency ACP control', () => {
    it.each(models)(
      '$model releases one active resident through standard session/close',
      async (model) => {
        if (!model.baseURL) {
        ...
        const root = await mkdtemp(
          path.join(
            os.tmpdir(),
            `blade-session-residency-acp-${safeSlug(model.model)}-`
          )
        );
        ...
        try {
          ...
          expect(evidence.capacityMessage).toContain(
          ...
          const [primaryTranscript, secondaryTranscript, storageText] =
            await Promise.all([
              readFile(
