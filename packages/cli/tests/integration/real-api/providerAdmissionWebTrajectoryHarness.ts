...
const scenarios = {
  queued: {
    suiteTitle: 'bounded fair Provider admission Web control',
    testTitle: '$model serializes two production Web Sessions',
    rootPrefix: 'blade-provider-admission-web',
    sessionTitle: 'Provider admission qualification',
    primaryMarkerPrefix: 'WEB_ADMISSION_PRIMARY',
    secondaryMarkerPrefix: 'WEB_ADMISSION_SECONDARY',
    workspacePaddingBytes: 0,
    pendingBytes: undefined,
  },
  weighted: {
    suiteTitle: 'bounded weighted Provider admission Web control',
    testTitle: '$model rejects a second production Web Session before Provider traffic',
    rootPrefix: 'blade-weighted-admission-web',
    sessionTitle: 'Weighted Provider admission qualification',
    primaryMarkerPrefix: 'WEIGHTED_WEB_PRIMARY',
    secondaryMarkerPrefix: 'WEIGHTED_WEB_SECONDARY',
    workspacePaddingBytes: 30 * 1024,
    pendingBytes: 64 * 1024,
  },
} as const;

export type ProviderAdmissionScenario = keyof typeof scenarios;
...
export function defineProviderAdmissionWebTrajectory(
  scenarioName: ProviderAdmissionScenario
): void {
  const scenario = scenarios[scenarioName];
  const weighted = scenarioName === 'weighted';

  describe.skipIf(!isRealApiTestEnabled()).sequential(scenario.suiteTitle, () => {
    it.each(models)(
      scenario.testTitle,
      async (model) => {
        if (!model.baseURL)
        ...
        const root = await mkdtemp(
          path.join(os.tmpdir(), `${scenario.rootPrefix}-${safeSlug(model.model)}-`)
        );
        ...
        try {
          ...
          if (scenario.workspacePaddingBytes > 0) {
            await writeFile(
              path.join(workspace, 'AGENTS.md'),
              `# Weighted admission fixture\n\n<!-- ${'x'.repeat(
                scenario.workspacePaddingBytes
            ...
            `${JSON.stringify(
              {
                ...
                ...(scenario.pendingBytes
                  ? { providerRequestPendingBytes: scenario.pendingBytes }
                  : {}),
          ...
          const primarySessionId = await createSession(
            origin,
            workspace,
            scenario.sessionTitle
          );
          const primaryMarker = `${scenario.primaryMarkerPrefix}_${Date.now()}`;
          ...
          const secondarySessionId = await createSession(
            origin,
            workspace,
            scenario.sessionTitle
          );
          ...
          await composer.fill(
            weighted
              ? `This request must be rejected before it reaches the Provider. ${secondaryMarker}`
              : `Reply with exactly ${secondaryMarker} and no other text.`
          );
