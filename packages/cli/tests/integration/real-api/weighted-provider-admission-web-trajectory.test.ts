...
describe
  ...
  .sequential('bounded weighted Provider admission Web control', () => {
    it.each(models)(
      '$model rejects a second production Web Session before Provider traffic',
      async (model) => {
        if (!model.baseURL)
        ...
        const root = await mkdtemp(
          path.join(
            os.tmpdir(),
            `blade-weighted-admission-web-${safeSlug(model.model)}-`
          )
        );
        ...
        try {
          ...
          if ((await page.locator('body').textContent())?.includes('Capacity queue')) {
            throw new Error(
              'Weighted Web reload restored transient Provider admission'
            );
          }
