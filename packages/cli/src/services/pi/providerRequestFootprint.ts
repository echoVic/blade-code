...
export const PROVIDER_REQUEST_FOOTPRINT_MAX_NODES = 100_000;
...
export function estimateRetainedValueBytes(
  ...
  const maxBytes = options.maxBytes ?? MAX_PROVIDER_REQUEST_PENDING_BYTES;
  const maxNodes = options.maxNodes ?? PROVIDER_REQUEST_FOOTPRINT_MAX_NODES;
  if (
