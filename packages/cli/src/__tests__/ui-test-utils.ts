...
export class UIComponentTestUtils {
   ...
   * 创建模拟键盘事件
   */
  static createKeyboardEvent(key: string, options: KeyboardEventInit = {}): KeyboardEvent {
    ...
    if (!isMatch) {
      ...
      for (let i = 0; i < maxLines; i++) {
        ...
        if (currentLine !== expectedLine) {
          differences.push(`Line ${i + 1}: expected "${expectedLine}", got "${currentLine}"`);
        }
      }
    }
...
export class UITestDataFactory {
   ...
   * 生成测试错误数据
   */
  static createError(message: string = 'Test error', code: string = 'TEST_ERROR'): Error {
