/**
 * useRefreshStatic Hook <p> 处理终端原生滚动模式下的 Static 组件刷新 <p> 背景： - 当 alternateBuffer: false
 * 时，使用终端原生滚动 - Ink 的 Static 组件内容会累积在终端缓冲区 - 需要在关键时刻（如终端 resize）清屏 + 重新挂载 Static
 */
 ...
 * 刷新 Static 组件的 Hook <p> 功能： - 清除终端屏幕 - 增加 clearCount 以强制 Static 组件重新挂载 <p> 触发时机： -

 * 终端宽度变化（由 useTerminalWidth 统一防抖） - 手动调用 refreshStatic()
...
export function useRefreshStatic() {
  ...
  /** 刷新 Static 组件 - 清屏（使用 ANSI 转义序列） - 增加 clearCount 强制 Static 重新挂载 */
