...
export const InputManager: React.FC<InputManagerProps> = ({
  ...
  const handleSubmit = useCallback(() => {
    ...
    switch (type) {
      ...
      case 'confirm':
        onSubmit(inputValue.toLowerCase() === 'y' || inputValue.toLowerCase() === 'yes');
        break;
    ...
    setIsSubmitted(true);
  }, [type, inputValue, selectedChoice, selectedChoices, choices, min, max, validate, onSubmit]);
      ...
      <Box flexDirection="column" marginTop={1}>
        {choices.map((choice, index) => (
          <Box key={index} flexDirection="row" alignItems="center">
            ...
            <Text
              color={choice.disabled ? 'gray' : index === selectedChoice ? 'blue' : 'white'}
              dim={!!choice.disabled}
              marginLeft={1}
            >
              {choice.name}
