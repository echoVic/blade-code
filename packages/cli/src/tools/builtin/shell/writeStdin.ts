...
const writeStdinSchema = Type.Refine(
  Type.Object({
    shell_id: Type.String({
      minLength: 1,
      description: 'Background shell ID returned by Bash',
    }),
    data: Type.Refine(
      Type.String({
        maxLength: MAX_STDIN_BYTES,
        description:
          'Exact text to write. Include a newline when the process expects one.',
      }),
      (value) => Buffer.byteLength(value) <= MAX_STDIN_BYTES,
      () => `data must be at most ${MAX_STDIN_BYTES} bytes`
    ),
    close_stdin: Default(
      Type.Boolean({
        description: 'Close stdin after writing so programs waiting for EOF can finish',
      }),
      false
    ),
  }),
  ({ data, close_stdin }) => data.length > 0 || close_stdin,
  () => 'data cannot be empty unless close_stdin is true'
);
