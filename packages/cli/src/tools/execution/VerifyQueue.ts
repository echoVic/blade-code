...
export class VerifyQueue {
  ...
  private readonly runCommand: VerifyQueueOptions['runCommand'];
  ...
  async verify(
    ...
    const run = async (): Promise<VerifyResult> => {
      const result = await this.runTypeCheck(workspace, signal);
  ...
  private async runTypeCheck(
    ...
    const command = buildTypeCheckArgs(workspaceRoot);
 ...
 *
 * 策略:
 * 1. 若 workspace 有 package.json 且有 scripts.type-check，则按
 *    packageManager/lockfile 选择包管理器执行该 script。
 * 2. 未声明脚本时不执行。自动下载或猜测工具会绕过用户的执行
 *    约定，也会让一次文件编辑隐式产生网络和供应链副作用。
 */
  ...
  if (existsSync(pkgPath)) {
    try {
      ...
      if (pkg?.scripts?.['type-check']) {
        return {
          cmd: resolvePackageManager(workspaceRoot, pkg.packageManager),
          args: ['run', 'type-check'],
        };
      }
    } catch {
...
async function defaultRunCommand(): Promise<{
  stdout: string;
  stderr: string;
  exitCode: number;
  timedOut: boolean;
}> {
  throw new Error('VerifyQueue requires a Session-owned command runner');
}
