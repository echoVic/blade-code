  ...
  describe('日志记录功能', () => {
    ...
    test('错误日志记录', () => {
      expect(() => {
        ...
        loggerComponent.error('error message with metadata', new Error('test error'), { code: '500' });
      }).not.toThrow();
    });
    ...
    test('致命错误日志记录', () => {
      expect(() => {
        ...
        loggerComponent.fatal('fatal message with metadata', new Error('fatal error'), { critical: true });
      }).not.toThrow();
    });
  });
