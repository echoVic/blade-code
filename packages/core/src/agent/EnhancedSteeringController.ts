...
class PriorityAsyncMessageQueue implements AsyncMessageQueue {
  ...
  push(message: MessageEvent): void {
    ...
    const insertIndex = this.messages.findIndex(msg => 
      priorityValue[msg.priority] < priorityValue[message.priority]
    );
...
class DefaultTaskInterceptor implements TaskInterceptor {
  private readonly interceptableTaskTypes: string[] = ['code_generate', 'code_review', 'file_operation'];
...
export class EnhancedSteeringController extends EventEmitter {
  ...
  private async handleMessage(message: MessageEvent): Promise<void> {
    switch (message.type) {
      case 'steering':
        await this.handleSteeringMessage(message as MessageEvent & { payload: SteeringDirection });
        break;
      case 'status':
   ...
   * 处理Steering控制消息
   */
  private async handleSteeringMessage(message: MessageEvent & { payload: SteeringDirection }): Promise<void> {
   ...
   * 向任务队列发送Steering指令
   */
  public sendSteeringCommand(direction: SteeringDirection, priority: 'high' | 'medium' | 'low' = 'medium'): void {
