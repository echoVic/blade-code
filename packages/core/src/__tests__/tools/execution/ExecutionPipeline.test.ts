...
import { ToolKind, ToolErrorType } from '../../tools/types/ToolTypes.js';
  ...
  describe('成功执行场景', () => {
    describe('当执行一个简单成功工具时', () => {
      ...
      it('应该在执行过程中触发正确的事件', async () => {
        ...
        const stageNames = ['discovery', 'validation', 'permission', 'confirmation', 'execution', 'formatting'];
        stageNames.forEach(stageName => {
          expect(stageStartedSpy).toHaveBeenCalledWith(
            expect.objectContaining({ stageName })
          );
          expect(stageCompletedSpy).toHaveBeenCalledWith(
            expect.objectContaining({ stageName })
          );
        });
  ...
  describe('失败执行场景', () => {
    ...
    describe('当工具执行本身失败时', () => {
      it('应该返回工具的错误信息', async () => {
        ...
        // Act: 执行会失败的工具
        const result = await pipeline.execute('failure_tool', { errorMessage: '测试错误' }, context);
  ...
  describe('边界情况和异常处理', () => {
    ...
    describe('当执行上下文异常时', () => {
      it('应该处理缺少signal的情况', async () => {
        ...
        await expect(
          pipeline.execute('success_tool', {}, context)
        ).rejects.toThrow();
      });
    });
  });
