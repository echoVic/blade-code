...
class WebFetchToolInvocation extends BaseToolInvocation<WebFetchParams> {
  ...
  async execute(signal: AbortSignal, updateOutput?: (output: string) => void): Promise<ToolResult> {
    try {
      ...
      if (response.status >= 400) {
        return this.createErrorResult(`HTTP错误 ${response.status}: ${response.status_text}`, {
          ...metadata,
  ...
  private async performRequest(options: {
    ...
    const { url, method, headers, body, timeout, follow_redirects, max_redirects, signal } =
      options;
...
export class WebFetchTool extends DeclarativeTool<WebFetchParams> {
  ...
  build(params: WebFetchParams): ToolInvocation<WebFetchParams> {
    ...
    const returnHeaders = this.validateBoolean(params.return_headers ?? false, 'return_headers');
