    ...
    try {
      ...
      if (response.status >= 400) {
        return this.createErrorResult(
          `API调用失败 ${response.status}: ${response.status_text}`,
          {
            ...metadata,
  ...
  build(params: ApiCallParams): ToolInvocation<ApiCallParams> {
    ...
    if (params.query_params !== undefined) {
      ...
      for (const [key, value] of Object.entries(params.query_params)) {
        queryParams[key] = this.validateString(value, `query_params.${key}`, {
          required: true,
        });
      }
    }
      ...
      switch (authType) {
        case 'bearer':
          if (params.auth.token) {
            auth!.token = this.validateString(params.auth.token, 'auth.token', {
              required: true,
            });
          }
          break;
        case 'basic':
          if (params.auth.username) {
            auth!.username = this.validateString(
          ...
          if (params.auth.password) {
            auth!.password = this.validateString(
    ...
    const parseResponse = this.validateBoolean(
      params.parse_response ?? true,
      'parse_response'
    );
