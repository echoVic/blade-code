      ...
      itemValidator: (item: any, index: number): EditOperation => {
        if (typeof item !== 'object' || item === null) {
          this.createValidationError(
            'edits',
            `编辑操作 ${index + 1} 必须是对象类型`,
            item
          );
        }

        const oldString = this.validateString(
          item.old_string,
          `edits[${index}].old_string`,
          {
            required: true,
            minLength: 1,
          }
        ...
        const newString = this.validateString(
          item.new_string,
          `edits[${index}].new_string`,
          {
            required: true,
          }
        );

        const replaceAll = this.validateBoolean(
          item.replace_all ?? false,
          `edits[${index}].replace_all`
        );
