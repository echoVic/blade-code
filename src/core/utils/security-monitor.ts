...
export interface SecurityEvent {
  id: string;
  timestamp: string;
  type: 'auth_failure' | 'path_traversal' | 'command_injection' | 'prompt_injection' | 'config_access' | 'rate_limit' | 'suspicious_activity';
...
export class SecurityMonitor {
  ...
  logEvent(event: Omit<SecurityEvent, 'id' | 'timestamp'>): void {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn(`[SECURITY] ${securityEvent.type}: ${JSON.stringify(securityEvent.details)}`);
    }
  }
  ...
  private sanitizeEventDetails(details: Record<string, any>): Record<string, any> {
    ...
    for (const [key, value] of Object.entries(details)) {
      // 脱敏敏感字段
      if (['password', 'token', 'key', 'secret', 'api', 'auth'].some(sensitive => 
  ...
  private triggerAlert(event: SecurityEvent): void {
    ...
    console.error(`[SECURITY ALERT] Critical event detected: ${event.type} from ${event.source}`);
