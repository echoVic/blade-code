...
export class PromptSecurity {
  ...
  static createSecurePrompt(
    ...
    for (const [key, value] of Object.entries(variables)) {
      ...
      const sanitizedValue = strictMode 
        ? this.sanitizeUserInput(value)
        : value;
    ...
    if (strictMode) {
      ...
      while (index !== -1) {
        ...
        if (endIndex !== -1) {
          const placeholderContent = result.substring(index, endIndex + endDelim.length);
  ...
  static createIsolatedMessages(
    ...
    messages.push({
      role: 'system',
      content: systemPrompt + (addContextMarkers ? '\n\n=== START OF CONVERSATION ===' : ''),
    });
