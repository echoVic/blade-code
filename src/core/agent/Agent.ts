...
export class Agent extends EventEmitter {
   ...
   * 处理带工具调用的消息（私有方法）
   */
  private async processMessageWithTools(message: string, context: ChatContext): Promise<{
    message: string;
    ...
    try {
      ...
      if (tool.requiresConfirmation) {
        ...
        if (confirmationDetails) {
          console.log(`⚠️  Tool ${tool.name} requires confirmation:`, confirmationDetails.title);
          // 在实际实现中，这里应该弹出确认对话框
          // 暂时自动确认
        }
      }
