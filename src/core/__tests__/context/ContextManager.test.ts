...
import { ContextEntry, ContextConfig } from '../types.js';
...
describe('ContextManager', () => {
  ...
  describe('错误处理', () => {
    test('应该处理无效的上下文条目', () => {
      expect(() => contextManager.addEntry({} as any))
        .toThrow('Context entry must have a type and content');
      
      expect(() => contextManager.addEntry({
        type: 'test'
        // 缺少 content
      } as any)).toThrow('Context entry must have a type and content');
    });
