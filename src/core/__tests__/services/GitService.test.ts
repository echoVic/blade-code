...
describe('GitService', () => {
  ...
  it('should clone repository', async () => {
    ...
    const result = await gitService.clone('https://github.com/test/repo.git', '/test/local');
    
    expect(result.command).toContain('git clone');
    expect(spawn).toHaveBeenCalledWith('git', expect.arrayContaining([
      'clone',
  ...
  it('should commit changes', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith('git', ['commit', '-m', '测试提交'], expect.any(Object));
  });
  ...
  it('should create branch', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith('git', ['branch', 'feature-branch'], expect.any(Object));
  });
  ...
  it('should checkout branch', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith('git', ['checkout', 'feature-branch'], expect.any(Object));
  });
  ...
  it('should get commit log', async () => {
    const mockChildProcess = {
      ...
      stdout: { 
        on: jest.fn((event, callback) => {
          if (event === 'data') {
            callback('abc1234|张三|zhang@example.com|2023-01-01 12:00:00 +0800|首次提交\n' +
  ...
  it('should handle git command errors', async () => {
    ...
    await expect(gitService.status('/invalid/path'))
      .rejects.toThrow('Git命令失败');
  });
  ...
  it('should check if directory is git repository', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith('git', ['rev-parse', '--git-dir'], expect.any(Object));
  });
