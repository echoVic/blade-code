...
import { Logger, LogLevel } from '../../logger/EnhancedLogger.js';
...
describe('LoggerComponent', () => {
  ...
  describe('日志记录', () => {
    ...
    test('应该能够记录警告信息', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith('Warning message', { warning: true });
    });
  ...
  describe('错误处理', () => {
    test('应该在未初始化时拒绝调用方法', () => {
      expect(() => loggerComponent.debug('test'))
        .toThrow('LoggerComponent not initialized');
      
      expect(() => loggerComponent.info('test'))
        .toThrow('LoggerComponent not initialized');
      
      expect(() => loggerComponent.warn('test'))
        .toThrow('LoggerComponent not initialized');
      
      expect(() => loggerComponent.error('test'))
        .toThrow('LoggerComponent not initialized');
      
      expect(() => loggerComponent.fatal('test'))
        .toThrow('LoggerComponent not initialized');
    });
