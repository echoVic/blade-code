...
class ShellToolInvocation extends BaseToolInvocation<ShellParams> {
  ...
  async shouldConfirm(): Promise<ConfirmationDetails | null> {
    ...
    const dangerousCommands = [
      ...
      'dd', 'shred', 'wipe', 'sudo', 'su', 'chmod',
      'chown', 'passwd', 'useradd', 'userdel', 'groupadd',
      'groupdel', 'systemctl', 'service', 'reboot', 'shutdown'
    ];
  ...
  private async executeCommand(options: {
    ...
    return new Promise((resolve, reject) => {
      const { command, args, cwd, timeout, env, capture_stderr, signal, updateOutput } = options;
...
export class ShellTool extends DeclarativeTool<ShellParams> {
  ...
  build(params: ShellParams): ToolInvocation<ShellParams> {
    ...
    const captureStderr = this.validateBoolean(params.capture_stderr ?? true, 'capture_stderr');
