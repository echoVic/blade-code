...
class TaskManager {
  ...
  updateTaskStatus(taskId: string, status: TaskStatus, data?: Partial<TaskResult>): void {
    ...
    if (task) {
      ...
      if ((status === TaskStatus.COMPLETED || status === TaskStatus.FAILED) && !task.completed_at) {
    ...
    if (run_in_background || subagent_type) {
      return {
        ...
        risks: [
          ...
          subagent_type ? `将使用${subagent_type}代理执行任务` : '将使用默认代理执行任务',
          '任务可能消耗系统资源',
        ],
        affectedFiles: [],
      };
