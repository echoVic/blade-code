...
class FindToolInvocation extends BaseToolInvocation<FindParams> {
  ...
  private shouldExclude(relativePath: string, name: string, excludePatterns: string[]): boolean {
...
export class FindTool extends DeclarativeTool<FindParams> {
  ...
  build(params: FindParams): ToolInvocation<FindParams> {
    ...
    if (!['file', 'directory', 'both'].includes(type)) {
      this.createValidationError('type', '类型必须是 file、directory 或 both 之一', type);
    }
    ...
    if (params.modified_after !== undefined) {
      ...
      try {
        ...
        this.createValidationError('modified_after', '时间格式无效，请使用ISO格式', modifiedAfter);
      }
    }
    ...
    const caseSensitive = this.validateBoolean(params.case_sensitive ?? false, 'case_sensitive');
