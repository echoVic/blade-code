...
export class SecurityPolicyManager {
  ...
  private loadDefaultPolicy(): SecurityPolicy {
    return {
      ...
      network: {
        allowedHosts: ['api.openai.com', 'api.anthropic.com', 'localhost', '127.0.0.1'],
  ...
  private savePolicy(): void {
    try {
      ...
      if (this.policyFile.startsWith(configDir)) {
        writeFileSync(this.policyFile, JSON.stringify(this.policy, null, 2), 'utf8');
      }
    } catch (error) {
  ...
  private checkPromptSecurity(data: any, violations: string[]): void {
    ...
    if (data?.prompt && typeof data.prompt === 'string') {
      if (data.prompt.length > policy.maxPromptLength) {
        violations.push(
          `提示词长度超过限制 (${data.prompt.length} > ${policy.maxPromptLength})`
        );
      }
...
export function RequirePermission(resource: string, action: string) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: any[]) {
      ...
      const access = securityPolicyManager.validateAccess(
        resource,
        action,
        userContext
      );
