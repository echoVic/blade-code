...
export class MemoryPool<T> {
  ...
  private shrink(): void {
    const shrinkCount = Math.max(0, this.pool.length - this.config.shrinkThreshold);
  ...
  private detectLeaks(): void {
    ...
    if (leakCandidates.length > this.config.threshold) {
      ...
      leakCandidates.slice(0, 5).forEach((obj) => {
        console.warn(
          `- ${obj.type} (ID: ${obj.id}, 存活时间: ${(Date.now() - obj.createTime) / 1000}s)`
        );
        if (obj.stackTrace) {
          console.warn(`  创建堆栈:\n${obj.stackTrace}`);
        }
