...
export class PerformanceMonitor extends EventEmitter {
  ...
  private analyzePerformance(): void {
    ...
    // 请求响应时间警告
    if (
      this.appMetrics.requests.averageResponseTime > this.config.thresholds.responseTime
    ) {
      this.addWarning({
  ...
  recordRequest(responseTime: number, success: boolean): void {
    ...
    const totalResponseTime =
      this.appMetrics.requests.averageResponseTime *
        (this.appMetrics.requests.total - 1) +
      responseTime;
    this.appMetrics.requests.averageResponseTime =
      totalResponseTime / this.appMetrics.requests.total;
  ...
  recordLLMCall(latency: number, cacheHit: boolean, error: boolean): void {
    ...
    if (error) {
      ...
      const errorCount =
        (this.appMetrics.llm.errorRate / 100) * (totalCalls - 1) + (error ? 1 : 0);
      this.appMetrics.llm.errorRate = (errorCount / totalCalls) * 100;
    }

    // 更新平均延迟
    const totalLatency =
      this.appMetrics.llm.averageLatency * (this.appMetrics.llm.totalCalls - 1) +
      latency;
  ...
  startTrace(
    name: string,
    category: PerformanceTrace['category'],
    parentId?: string
  ): string {
      ...
      system: {
        memoryUsage: latest
          ? {
              ...
              rss: latest.memoryUsage.rss,
              percentage: (
                (latest.memoryUsage.heapUsed / latest.memoryUsage.heapTotal) *
                100
              ).toFixed(2),
            }
          : null,
