...
export const withTheme = <P extends object>(
  ...
  const ThemedComponent: React.FC<P & { variant?: string }> = ({
    variant,
    ...props
  }) => {
    ...
    return (
      <Component {...(props as P)} style={{ ...style, ...(props as any).style }} />
    );
  };
...
export const ThemedText: React.FC<{
  ...
  const getVariantColor = () => {
    switch (variant) {
      case 'primary':
        return theme.tokens.colors.semantic.primary[500];
      case 'secondary':
        return theme.tokens.colors.semantic.secondary[500];
      case 'success':
        return theme.tokens.colors.semantic.success[500];
      case 'warning':
        return theme.tokens.colors.semantic.warning[500];
      case 'error':
        return theme.tokens.colors.semantic.error[500];
      case 'info':
        return theme.tokens.colors.semantic.info[500];
      default:
        return theme.tokens.colors.text.primary;
    }
  };

  const finalColor = color || getVariantColor();

  return <span style={{ color: finalColor, ...style }}>{children}</span>;
};
...
export const ThemedBox: React.FC<{
  ...
  return <div style={{ ...variantStyle, ...style }}>{children}</div>;
};
