...
class ShortTermMemory {
  ...
  public async cleanupExpired(): Promise<void> {
    ...
    for (const [id, data] of this.contexts) {
      if (
        now - data.timestamp > this.maxDuration ||
...
class LongTermMemory {
  ...
  private searchIndex = new Map<string, string[]>(); // 搜索索引
  private readonly maxStorageSize: number; // MB
  private storage = 0;

  constructor(maxStorageSize = 1000) {
    // 1GB 默认限制
  ...
  async search(query: SearchQuery): Promise<SearchResult[]> {
    ...
    const scoredResults: SearchResult[] = candidates.map((item) => {
      ...
      // 时间衰减
      const age = Date.now() - (item.lastUpdated || item.timestamp);
  ...
  private async updateIndex(data: PersistentKnowledge): Promise<void> {
    ...
    for (const word of uniqueWords) {
      ...
      this.searchIndex.get(word)!.push(data.id);
    }
  }
...
export class LayeredMemoryManager extends EventEmitter {
  ...
  private async migrateMediumToLongTerm(): Promise<number> {
    const mediumTermSessions = Array.from(
      (this.mediumTermMemory as any).sessions.values()
    );
    let migratedCount = 0;

    for (const context of mediumTermSessions) {
      const contextData = context as ContextData;
      if (this.migrationStrategy.mediumToLong.condition(contextData)) {
        // 生成摘要
        const _summary = this.migrationStrategy.mediumToLong.summary(contextData);

        // 提取知识
        const knowledgeItems = this.migrationStrategy.mediumToLong.extract(contextData);
  ...
  public async getMemoryStats(): Promise<{
    ...
    const longTermKnowledge = await this.longTermMemory.search({
      // 获取所有长期知识
      query: '',
      layer: MemoryLayer.LONG_TERM,
      maxResults: 1000000,
    ...
    stats.totalMemory =
      stats.shortTerm.totalSize + stats.mediumTerm.totalSize + stats.longTerm.totalSize;
  ...
  public async cleanup(): Promise<{
    ...
    try {
      ...
      for (const session of mediumTermSessions) {
        const sessionData = session as ContextData;
        if (now - sessionData.timestamp > this.config.mediumTerm.maxDuration) {
          await (this.mediumTermMemory as any).sessions.delete(sessionData.id);
    ...
    return {
      ...
      priority:
        targetLayer === MemoryLayer.SHORT_TERM
          ? 100
          : targetLayer === MemoryLayer.MEDIUM_TERM
            ? 50
            : 25,
    };
  }
