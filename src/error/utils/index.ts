 ...
 * 判断错误是否为特定模块
 */
export function isErrorFromModule(
  error: Error | BladeError,
...
export function filterErrorsByModule(
  ...
  return errors.filter(isBladeError).filter((error) => error.module === module);
}
...
export function filterErrorsByCategory(
  ...
  return errors.filter(isBladeError).filter((error) => error.category === category);
}
...
export function filterErrorsBySeverity(
  ...
  return errors.filter(isBladeError).filter((error) => error.severity === severity);
}
...
export function getRetryableErrors(errors: Array<Error | BladeError>): BladeError[] {
  return errors.filter(isBladeError).filter((error) => error.isRetryable());
}
...
export function getRecoverableErrors(errors: Array<Error | BladeError>): BladeError[] {
  return errors.filter(isBladeError).filter((error) => error.isRecoverable());
}
...
export function createErrorChain(...errors: Array<Error | BladeError>): BladeError {
  ...
  const bladeError = isBladeError(lastError) ? lastError : BladeError.from(lastError);
 ...
 * 获取最相关的错误（按严重程度排序）
 */
export function getMostRelevantError(
  errors: Array<Error | BladeError>
): Error | BladeError {
  if (errors.length === 0) {
...
export async function safeExecute<T>(
  ...
  try {
    ...
    const bladeError = error instanceof Error ? error : new Error(String(error));
