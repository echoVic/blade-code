...
export class RetryManager {
  ...
  async execute<T>(operation: () => Promise<T>, operationId?: string): Promise<T> {
    ...
    while (retryState.attempts < this.retryConfig.maxAttempts) {
      try {
        ...
        // 记录重试信息
        console.warn(
          `重试操作 "${id}" (尝试 ${retryState.attempts}/${this.retryConfig.maxAttempts})，延迟 ${retryState.nextDelay}ms`
        );
  ...
  async executeWithTimeout<T>(
    ...
    return Promise.race([
      ...
      new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(
            new BladeError(ErrorCodeModule.CORE, '0004', `操作 "${id}" 超时`, {
  ...
  private calculateDelay(attempts: number): number {
    let delay =
      ...
      Math.pow(this.retryConfig.backoffFactor, attempts - 1);
