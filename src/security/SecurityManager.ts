...
class InputValidationLayer implements SecurityLayer {
  ...
  async process<T>(request: SecurityRequest<T>): Promise<SecurityRequest<T>> {
    ...
    try {
      ...
      request.metadata.auditTrail = [
        auditEvent,
        ...((request.metadata.auditTrail as AuditEvent[]) || []),
      ];
  ...
  private async validateAndSanitizeInput(input: string): Promise<string> {
    ...
    if (input.length > this.config.maxInputLength) {
      throw new Error(
        `输入长度超过最大限制: ${input.length} > ${this.config.maxInputLength}`
      );
    }
...
class AuthenticationLayer implements SecurityLayer {
  ...
  private async authenticateUser(context: SecurityContext): Promise<boolean> {
    ...
    if (context.metadata?.credentials) {
      const { username, password } = context.metadata.credentials as Record<
        string,
        string
      >;
...
class AuthorizationLayer implements SecurityLayer {
  ...
  async process<T>(request: SecurityRequest<T>): Promise<SecurityRequest<T>> {
    ...
    const auditEvent: AuditEvent = {
      ...
      data: {
        ...
        operation: request.context.operationType,
      },
      result: true,
    };
  ...
  private getRequiredPermissions(context: SecurityContext): string[] {
    ...
    return [
      operationToPermission[
        context.operationType as keyof typeof operationToPermission
      ] || 'read',
    ];
  }
...
class DataSanitizationLayer implements SecurityLayer {
  ...
  constructor(config: Partial<DataSanitizationConfig>) {
    this.config = {
      ...
      failureAction: config.failureAction ?? 'block',
      sanitizationRules:
        config.sanitizationRules ?? this.createDefaultSanitizationRules(),
...
class AuditLoggingLayer implements SecurityLayer {
  ...
  async process<T>(request: SecurityRequest<T>): Promise<SecurityRequest<T>> {
    ...
    try {
      ...
      // 审计日志失败不应阻止请求处理
      console.error(
        `[AuditLoggingLayer] 审计日志记录失败: ${(error as Error).message}`
      );
      return request;
    }
  }
  ...
  private determineEntryLevel(
    entry: AuditEvent
  ): 'debug' | 'info' | 'warning' | 'error' {
    if (entry.error) return 'error';
    if (entry.result === false) return 'warning';
    return 'info';
  }
...
export class SecurityManager extends EventEmitter {
      ...
      for (const layer of this.securityLayers) {
        try {
          ...
          if (processedRequest.blocked) {
            this.log(
              `请求在安全层级 ${layer.getLevel()} 被阻止`,
              processedRequest.blockedReason
            );
            ...
            const response: SecurityResponse<T> = {
              ...
              auditTrail: (processedRequest.metadata.auditTrail as AuditEvent[]) || [],
  ...
  private createDefaultLayerConfigs(): Record<string, Partial<SecurityLayerConfig>> {
    return {
      inputValidation: {
        enabled: true,
        strictMode: true,
  ...
  private createSuccessResponse<T>(
    ...
    return {
      ...
      auditTrail: (request.metadata.auditTrail as AuditEvent[]) || [],
      securityLevels:
        levels.length > 0 ? levels : this.securityLayers.map((l) => l.getLevel()),
    };
  }
  ...
  private calculateSecurityScore(
    layer: SecurityLayer,
    request: SecurityRequest<unknown>
  ): number {
    // 简化的安全评分计算
    const baseScore = 1.0;
