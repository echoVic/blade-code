...
### 修复
- worktree 相关的 Git 命令改为在 C locale 下执行，使 Git 输出本地化报错时，在非 Git 目录中清理过期 worktree 仍保持无操作。

### 测试
- worktree 清理回归强制使用本地化的 Git 环境，只要 Git 带有翻译，缺少修复时即可复现失败。

## [0.11.4] - 2026-09-24

### 修复
- 将 `wrap-ansi` 声明为直接运行时依赖；终端 Markdown 表格渲染直接引用了它，此前 CLI 只能通过 `ink` 与 `yargs` 间接获得。

### 重构
- 清理 `knip` 报告的未使用导出、类型与脚本，包括在 `knip` 发现问题时仍报告成功的 `detect-unused` 包装脚本，以及已被取代的 `scripts/ready.ts`。
- 移除 23 个根工作区依赖，它们会把更高的主版本提升到各包声明的版本之上。

### 测试
- 在清理后的依赖布局上验证类型检查、lint、生产构建与单元测试。

## [0.11.3] - 2026-09-22

### 新功能
- 在已完成和已中止的 Session 事件中持久化每轮输入、输出、缓存读取、缓存写入、新输入和预估成本指标。
- 为 Web 日志新增运行、模型、工具和缓存四条泳道的时间轴，并在 hover 与键盘聚焦时展示详细指标。

### 变更
- 保留持久消息和工具时间戳，使重新打开 Session 后仍能展示真实执行时间分布，而不是按加载时间重建。
- 保留现有可搜索工具日志作为“明细”视图，并在两种视图上方汇总成本、缓存命中率、缓存输入和新输入。

### 测试
- 覆盖用量规范化与聚合、持久化完成与崩溃恢复、历史时间戳、时间区间推导、指标渲染及桌面/移动端响应式行为。
- 验证完整单元与 Web 测试、lint、类型检查、生产构建和浏览器渲染；未运行付费真实 API 资格测试。

## [0.11.2] - 2026-09-22

### 新功能
- 为长任务新增自动行内 Conversation recap，在 TUI、Web、ACP 和 Headless 中概括当前目标、已验证进展、阻塞项与下一步。
- 将回顾作为仅展示的会话历史持久化，同时从后续模型上下文、工具执行、结构化输出和压缩检查点中排除。

### 变更
- 按经过时间和已完成模型轮次限制回顾频率，使用有界无工具 Provider 请求；生成或持久化失败时静默继续主任务。
- 使用低强调行内文本展示回顾，并在实时流、断线重放、会话恢复及后续模型轮次之间保持稳定顺序。

### 测试
- 覆盖回顾调度、输入输出预算、取消、持久化、重放、Token 计量、提示隔离与失败恢复。
- 验证 TUI 流式顺序、Web 响应式渲染、ACP 历史投影、Headless JSONL 输出，以及全量回归、构建、lint 和类型检查门禁。
