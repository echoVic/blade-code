...
describe('MCPComponent', () => {
  ...
  describe('消息处理', () => {
    ...
    test('应该在未连接时拒绝发送消息', async () => {
      ...
      await expect(mcpComponent.sendRequest({ method: 'test' })).rejects.toThrow(
        'MCP client not connected'
      );

      await expect(mcpComponent.sendNotification({ method: 'test' })).rejects.toThrow(
        'MCP client not connected'
      );
    });
  });
  ...
  describe('错误处理', () => {
    test('应该在未初始化时拒绝调用方法', async () => {
      await expect(mcpComponent.connect()).rejects.toThrow(
        'MCPComponent not initialized'
      );

      await expect(mcpComponent.disconnect()).rejects.toThrow(
        'MCPComponent not initialized'
      );
      ...
      expect(() => mcpComponent.isConnected()).toThrow('MCPComponent not initialized');
    });
